...
void ExampleFeatures1(float * pSpeed, int * pTimesToRenderScene, ovrVector3f * useHmdToEyeViewOffset)
{
    ...
    if (WND.Key['1']) MakeNewDistortionMeshes(0.001f); // Min eye relief
    if (WND.Key['2']) MakeNewDistortionMeshes(1.000f); // Max eye relief
    ...
    if (WND.Key['M'])                         *pSpeed *= 2;    
    if (((clock % 2) != 0) && (WND.Key['M'])) *pSpeed = 0;
#endif
}
                      ...
                      bool * pClearEyeImage,bool * pUpdateEyeImage)
{
    ...
    if ((WND.Key['N']) && ((clock % (BLANK_FREQUENCY*2)) == (eye*BLANK_FREQUENCY)))
...
//-----------------------------------------------------------------------------------------
#if RENDER_OPENGL
void ExampleFeatures3(const ShaderFill::VertexAttribDesc * VertexDesc, int numVertexDesc,
                      ...
                      float ** pUseYaw, double * pDebugTimeAdjuster,bool * pWaitForGPU)
{
    ...
    if ((WND.Key['M']) && (((clock % 4) == 0) || ((clock % 4) == 3)) && (eye == 0))   
    {
        *pUseEyePose    = &extraRenderPose;
        *pUseYaw        = &extraYaw;
    ...
    // Non SDK-rendered only.
    if (WND.Key['4']) *pDebugTimeAdjuster = -0.026; // Greatly underpredicting
    if (WND.Key['5']) *pDebugTimeAdjuster = -0.006; // Slightly underpredicting
    if (WND.Key['6']) *pDebugTimeAdjuster = +0.006; // Slightly overpredicting
    if (WND.Key['7']) *pDebugTimeAdjuster = +0.026; // Greatly overpredicting
